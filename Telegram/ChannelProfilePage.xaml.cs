...
namespace Telegram
{
    public sealed partial class ChannelProfilePage : Page
    {
        ...
        {
            if (chat == null) return;
            ProfileTitle.Text = string.IsNullOrEmpty(chat.Title) ? "Channel" : chat.Title;
            ProfileSubtitle.Text = BuildSubscribersText(chat);
            ...
            var description = chat.Bio;
            if (string.IsNullOrEmpty(description)) description = _loadedFullInfo ? "No description." : string.Empty;
            SetRichText(DescriptionText, description);
            SetRichText(UsernameText, string.IsNullOrEmpty(chat.Username) ? string.Empty : "@" + chat.Username.TrimStart('@'));
        }

        private string BuildSubscribersText(ChatViewModel chat)
        {
            if (chat == null || chat.SubscriberCount <= 0) return "channel";
            return ChatViewModel.FormatCount(chat.SubscriberCount) + " subscribers";
        }

        private void SetRichText(RichTextBlock block, string text)
        {
            block.Blocks.Clear();
            ...
            for (var i = 0; i < parts.Length; i++)
            {
                ...
                {
                    paragraph.Inlines.Add(new Run { Text = text[index].ToString() });
                    index++;
                    continue;
                }

                var start = text.IndexOf(part, index, StringComparison.Ordinal);
                if (start > index) paragraph.Inlines.Add(new Run { Text = text.Substring(index, start - index) });
                AddTextPart(paragraph, part);
                index = start + part.Length;
            }
            if (index < text.Length) paragraph.Inlines.Add(new Run { Text = text.Substring(index) });
            block.Blocks.Add(paragraph);
        }

        private void AddTextPart(Paragraph paragraph, string part)
        {
            var link = NormalizeLink(part);
            if (!string.IsNullOrEmpty(link))
            {
                var h = new Hyperlink();
                try { h.NavigateUri = new Uri(link); } catch { }
                h.Inlines.Add(new Run { Text = part });
        ...
        private string NormalizeLink(string text)
        {
            if (string.IsNullOrEmpty(text)) return null;
            var clean = text.Trim().TrimEnd('.', ',', ')', ']');
            ...
            if (clean.StartsWith("@", StringComparison.Ordinal) && clean.Length > 1) return "https://t.me/" + clean.Substring(1);
            if (clean.StartsWith("t.me/", StringComparison.OrdinalIgnoreCase)) return "https://" + clean;
            return null;
        }
        ...
        private async void InfoTab_Tapped(object sender, TappedRoutedEventArgs e) { await SelectTabAsync("info"); }
        ...
        {
            _selectedTab = tab;
            InfoTab.Opacity = tab == "info" ? 1 : 0.68;
        ...
        private async System.Threading.Tasks.Task LoadHistoryAsync()
        {
            ...
            catch { _messages = new List<ChatMessageViewModel>(); _historyDone = true; }
            _historyLoaded = true;
        ...
        private async System.Threading.Tasks.Task LoadMoreAsync()
        {
            ...
            try
            {
                ...
                else { MergeMessages(older); _oldestMessageId = GetOldestMessageId(); }
            }
        ...
        private void FillContent(string tab)
        {
            ...
            EmptyContentText.Text = _contentItems.Count == 0 ? "No " + tab + " in this channel." : string.Empty;
        ...
        private void SortMessages() { _messages.Sort(delegate(ChatMessageViewModel a, ChatMessageViewModel b) { return (b == null ? 0 : b.Date).CompareTo(a == null ? 0 : a.Date); }); }
        private int GetOldestMessageId() { var min = 0; for (var i = 0; i < _messages.Count; i++) if (_messages[i] != null && _messages[i].Id > 0 && (min == 0 || _messages[i].Id < min)) min = _messages[i].Id; return min; }

        private async void LoadMoreContentButton_Click(object sender, RoutedEventArgs e) { await LoadMoreAsync(); FillContent(_selectedTab); }
